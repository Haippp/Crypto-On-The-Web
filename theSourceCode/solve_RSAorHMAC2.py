...
def create_session(username: str) -> str:
    url_session = URL + f'create_session/{username}/'
    return r.get(url_session).json()['session']

def auth(token):
    auth_url = URL + f'authorise/{token}/'
    response = r.get(auth_url).json()
    return response['response']

jwt0 = create_session('Haipp1')
jwt1 = create_session('Haipp2')
HASH = SHA256
...
pkcs1 = asn1tools.compile_files('theSourceCode/pkcs1.asn', codec='der')
x509 = asn1tools.compile_files('theSourceCode/x509.asn', codec='der')
jwts=[]
...
public_key = open(pem_name, 'r').read()
